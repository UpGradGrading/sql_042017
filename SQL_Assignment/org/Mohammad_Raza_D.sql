...
no_of_customers*/
SELECT region, count(1) no_of_customers 
FROM cust_dimen 
GROUP BY region ORDER BY no_of_customers DESC

/*C. Find the region having maximum customers (display the region name and
max(no_of_customers)*/
SELECT a.region, MAX(a.cust_count) FROM (
SELECT region, COUNT(customer_name) cust_count  FROM cust_dimen
...
SELECT prod_id "product id", SUM(order_quantity) "no_of_products sold" 
FROM market_fact
GROUP BY prod_id
...
purchased)*/
SELECT cd.customer_name, SUM(mf.order_quantity) no_of_tables_purchased
FROM cust_dimen cd,prod_dimen pd, market_fact mf
WHERE cd.cust_id = mf.cust_id  #INNER Join
AND pd.prod_id = mf.prod_id
AND cd.region ='ATLANTIC'
AND pd.product_sub_category = 'TABLES'
GROUP BY cd.customer_name
#ORDER BY mf.order_quantity DESC
...
FROM prod_dimen pd, market_fact mf
WHERE pd.prod_id = mf.prod_id #INNER Join
...
#C2 reagion of least profitable product
SELECT cd.region, sum(mf.profit),count(1)
FROM cust_dimen cd,prod_dimen pd, market_fact mf
WHERE cd.cust_id = mf.cust_id
AND pd.prod_id = mf.prod_id
AND pd.product_sub_category = 'TABLES'
GROUP BY cd.region
ORDER BY mf.profit DESC

## We can combine C1 and C2 and can get least profitable product and region in one query
SELECT cd.region, sum(mf.profit),count(1)
FROM cust_dimen cd,prod_dimen pd, market_fact mf
WHERE cd.cust_id = mf.cust_id
AND pd.prod_id = mf.prod_id
AND pd.product_sub_category IN (SELECT pd.product_sub_category
FROM prod_dimen pd, market_fact mf
WHERE pd.prod_id = mf.prod_id
GROUP BY pd.product_category,pd.product_sub_category
HAVING SUM(mf.profit) <=(SELECT MIN(a.profit1) FROM(
SELECT prod_id,SUM(profit) profit1 FROM market_fact 
GROUP BY prod_id) a)) # subquery
GROUP BY cd.region
ORDER BY mf.profit DESC
