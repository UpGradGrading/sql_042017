...
/* There are 5 datasets at hand. Four are dimension datasets that describe all the details of the respective entity. For example: cust_dimen provides all attributes required to describe a customer and orders_dim keeps all information for orders placed. 
 All datasets ending with '_dimen' present similar information for their entities.
 The fifth dataset 'market_fact' contains keys from all other tables along with relevant facts or metrics. This table acts as a single snapshot to calculate any metric.
...
An interesting thought about orders_dimen is that it has Order_id along with Ord_id. By Naming convention, either can act as the primary key of this dataset, but in reality, only Ord_id is as Order_id has duplicates. 
Order_id is just used to relate to shipments made. I believe it is a redundant attribute.

Another observation is that there are no units available for each attribute of each dataset. This always leads to confusion and is a bad practice.
...
select Region, count(cust_id) as cust_count from cust_dimen group by Region order by count(cust_id) desc;

-- 2.C Find the region having maximum customers (display the region name and max(no_of_customers)
select Region, count(cust_id) as cust_count from cust_dimen group by Region order by count(cust_id) desc limit 1;

--2.D Find the number and id of products sold in decreasing order of products sold (display product id, no_of_products sold)
select Prod_id, count(Order_Quantity) as no_of_products_sold from market_fact group by Prod_id order by no_of_products_sold desc;
...
--3.B Display the product category, product sub-category and the profit within each sub-category in three columns.
select 
